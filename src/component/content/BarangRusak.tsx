...
function BarangRusakBody(): React.JSX.Element {
  ...
  async function handleInspeksi(row: BarangRusak, jenis: 'retur' | 'rusak'): Promise<void> {
    const ok = await confirm({
      ...
      message:
        jenis === 'retur'
          ? 'Data Hasil pengecekan fisik akan dikunci sebagai "Bisa Diretur", barang akan diproses retur ke supplier.'
          : 'Data Hasil pengecekan fisik akan dikunci sebagai "Rusak", barang tidak bisa diretur ke supplier.',
  ...
  return (
    <PageShell title="Barang Rusak" breadcrumb="Manajemen / Barang Rusak">
      ...
      <DataTable
        ...
        description="Laporan barang rusak/retur status default 'Menunggu Pengecekan' sampai selesai pemrosesan pengecekan barang fisik oleh Admin/Super Admin"
        columns={columns}
        rows={rows}
        getRowId={(row) => row.id}
        isLoading={isLoading}
