...
export function GoodsOutContent(): React.JSX.Element {
  const rows = SEED_TRANSACTIONS.filter((row) => row.type === 'Keluar');
  ...
  return (
    <PageShell
      title="Barang Keluar"
      breadcrumb="Pengelolaan / Barang Keluar"
      action={<Button onClick={() => setIsModalOpen(true)}>+ Input Barang Keluar</Button>}
    >
      <StatsRow
        stats={[
